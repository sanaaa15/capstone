...
export async function POST(request: Request) {
    ...
    try {
        const { description, quantity, attributes } = await request.json();

        const [result] = await pool.execute(
            `INSERT INTO kurta_details (user_id, description, quantity, sleeve_length, color, hemline, neckline, print, sleeve_style)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
            [
                userId,
                description,
        ...
        return NextResponse.json({ 
            message: 'Kurta details added successfully',
            itemId: result.insertId
        });
    } catch (error) {
        console.error('Error adding kurta details:', error);
